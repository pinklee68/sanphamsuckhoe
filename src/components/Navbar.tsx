...
export default function Navbar({
  ...
  const handleGoogleLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const emailToUse = googleEmailInput.trim();
    if (!emailToUse) {
      setErrorMsg('Vui lòng nhập địa chỉ Email Google (Gmail) của bạn');
      return;
    }
    try {
  ...
  const handleGooglePopupLogin = async () => {
    setErrorMsg('');
    try {
      const user = await AuthService.loginWithGoogleReal();
      onUserChange(user);
      setShowAuthModal(false);
      setGoogleEmailInput('');
      setErrorMsg('');
    } catch (err: any) {
      console.warn('Google Popup auth error/fallback:', err);
      setErrorMsg('Popup Google bị chặn trong iFrame. Vui lòng nhập email của bạn vào ô trên để đăng nhập.');
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#030712]/80 backdrop-blur-sm animate-fade-in">
          <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-[#111827] border border-white/5 p-6 shadow-2xl">
            ...
            <p className="text-xs text-gray-400 mb-6">
              Nhập tài khoản Email Google của bạn để đăng nhập vào hệ thống.
            </p>

            {/* Google Email Login Form */}
            <form onSubmit={handleGoogleLoginSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-300 mb-1.5">
                  Email Google (Gmail) của bạn:
                </label>
                <input
                  type="email"
                  ...
                  placeholder="vd: lthongxanh@gmail.com"
                  className="w-full rounded-xl bg-[#030712] border border-white/15 px-4 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 transition-colors"
                  autoFocus
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2.5 rounded-xl bg-white hover:bg-gray-100 py-2.5 text-sm font-semibold text-gray-900 shadow-lg hover:shadow-xl transition-all cursor-pointer active:scale-[0.99]"
              >
            ...
            <button
              ...
              type="button"
              className="w-full py-2 px-3 rounded-lg border border-white/10 hover:border-white/20 text-xs text-gray-300 hover:text-white transition-all cursor-pointer flex items-center justify-center gap-2"
            >
              <span>Mở Popup Google OAuth tự động</span>
            </button>
          </div>
        </div>
      )}
